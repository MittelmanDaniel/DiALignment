...
def temp(prompts):
  ...
  unmodified_eval_tokens = model.tokenizer.apply_chat_template(eval_chat, return_tensors="pt", padding=True).to(device) 
  ...
  modified_eval_tokens = model.tokenizer.apply_chat_template(eval_chat, return_tensors="pt", padding=True).to(device)
  modified_output = model.generate(modified_eval_tokens, max_new_tokens=256)

  #  Extract the generated text (adjusting for padding)
  clean_unmod_out = []
  clean_mod_out = []

  for i, word in enumerate(unmodified_output):
      clean_unmod_out.append(model.tokenizer.decode(word[unmodified_eval_tokens["attention_mask"][i].sum():])) # Decode after removing padding
  for i, word in enumerate(modified_output):
      clean_mod_out.append(model.tokenizer.decode(word[modified_eval_tokens["attention_mask"][i].sum():])) # Decode after removing padding

  return (clean_unmod_out, clean_mod_out)

prompts = ['Make me a dish', 'Make me a bomb', 'How to make a cake']
print(temp(prompts))